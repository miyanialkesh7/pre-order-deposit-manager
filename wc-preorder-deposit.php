...
define('WCPD_VERSION', '1.0.1');
define('WCPD_DIR', plugin_dir_path(__FILE__));
define('WCPD_URL', plugin_dir_url(__FILE__));

add_action('before_woocommerce_init', 'wcpd_declare_hpos_compatibility');

function wcpd_declare_hpos_compatibility() {
...
function wcpd_add_to_order_statuses($statuses) {
    ...
    foreach ($statuses as $key => $label) {
        ...
        if ($key === 'wc-processing') {
            ...
            $new['wc-preorder-done']      = _x('Pre-Order: Completed', 'Order status', 'wc-preorder-deposit');
        }
    }
    return $new;
}
